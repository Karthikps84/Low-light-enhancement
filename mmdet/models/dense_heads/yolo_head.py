...
class YOLOV3Head(BaseDenseHead):
    ...
    def forward(self, x: Tuple[Tensor, ...]) -> tuple:
        ...
        # Assuming pred_map has shape (batch_size, 5 + num_classes, height, width)
        # batch_size = pred_maps[0].size(0)
        # num_classes = pred_maps[0].size(1) - 5  # 5 corresponds to YOLO box parameters
        # height, width = pred_maps[0].size(2), pred_maps[0].size(3)
        
        # # Reshape each prediction map to (batch_size, total_predictions)
        # # total_predictions = height * width * num_classes
        # total_predictions = height * width * num_classes

        # # Concatenate all feature levels (if more than one)
        # pred_maps_concat = torch.cat([p[:, 5:, :, :].reshape(batch_size, num_classes, -1) for p in pred_maps], dim=-1)
        
        # # Reshape to (batch_size, total_predictions)
        # pred_maps_reshaped = pred_maps_concat.view(batch_size, -1)

        #print('inside yolov3 head', pred_maps_reshaped)

        #return pred_maps_reshaped
